...
class Skor {
	...
	function readAlternatif($a, $b) {
		$query = "SELECT * FROM tbl_eduapp_alternative_data WHERE alt_id='$a' AND user_id='$b' ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
	...
	function readMulti2($a, $b, $c, $d) {
		$query = "SELECT POWER(EXP(SUM(LOG(alternative_analysis_value))), 1/'$d') AS multitotal FROM {$this->table_name} WHERE fld_alternative_1='$a' AND fld_subcriteria_id = '$b' AND user_id='$c'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	function readSubPri($a, $b) {
		$query = "SELECT * FROM tbl_eduapp_subcriteria_data AS subpri WHERE fld_subcriteria_id='$a' AND user_id='$b' ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->sub = $row['sub_global_priority'];
	...
	function readKri($a, $b) {
		$query = "SELECT * FROM tbl_eduapp_subcriteria_data AS rkri WHERE fld_subcriteria_id='$a' AND user_id='$b' ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->kri = $row['fld_subcriteria_name'];
	...
	function readKriId($a, $b) {
		$query = "SELECT * FROM tbl_eduapp_subcriteria_data AS criid WHERE fld_subcriteria_id='$a' AND user_id='$b' ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->criid = $row['fld_subcriteria_id'];
	...
	function getCI($a, $b) {
		$query = "SELECT sum(alt_em_total*alt_em) AS total FROM tbl_eduapp_alternative_details WHERE user_id='$a' AND fld_subcriteria_id='$b'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->nak = $row['total'];
	...
	function getCI2($a, $b) {
		$query = "SELECT sum(alt_an_total*alt_an) AS total FROM tbl_eduapp_alternative_details WHERE user_id='$a' AND fld_subcriteria_id='$b' ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->nak = $row['total'];
