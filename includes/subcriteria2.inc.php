...
class Subcriteria2 {
	private $conn;
	private $table_name = "tbl_eduapp_subcriteria_data_2";
	...
	function readAll() {
		$query = "SELECT * FROM tbl_eduapp_subcriteria_2 ORDER BY fld_subcriteria_id ASC";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
	...
	function readSatu($a) {
		$query = "SELECT * FROM tbl_eduapp_subcriteria_2 WHERE fld_subcriteria_id='$a' LIMIT 0,1";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
