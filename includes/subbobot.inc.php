...
class Subbobot {
	...
	function countAll() {
		$query = "SELECT * FROM tbl_eduapp_subcriteria";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
	...
	function getCI($a) {
		$query = "SELECT sum(fld_subcriteria_total*subcriteria_em) AS total FROM tbl_eduapp_subcriteria_data WHERE user_id='$a'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->nak = $row['total'];
	...
	function getCI2($a) {
		$query = "SELECT sum(fld_subcriteria_total*subcriteria_an) AS total FROM tbl_eduapp_subcriteria_data WHERE user_id='$a'";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->nak = $row['total'];
