      ...
      <thead>
        ...
        <tr class="success">
          <?php $bobot1 = $ranObj->readPrioC1($user); while ($row = $bobot1->fetch(PDO::FETCH_ASSOC)): ?>
            <td colspan="<?php $subcri1a = $subcriObj->readAll2($user, $row['fld_criteria_id']); echo $subcri1a->rowCount(); ?>" class="text-center"><?=number_format($row['criteria_an'], 4, '.', ',')?></td> 
          <?php endwhile; ?>
        ...
        <tr class="success">
          <?php $subbobot1 = $ranObj->readBob($user); while ($row = $subbobot1->fetch(PDO::FETCH_ASSOC)): ?>
            <td><?=number_format($row['global_priority_an'], 4, '.', ',')?></td> 
          <?php endwhile; ?>
        </tr>
          ...
          <tr>
            ...
            <?php $subcri2a = $subcriObj->readAllSubCri($user); while ($row2 = $subcri2a->fetch(PDO::FETCH_ASSOC)): ?>
              ...
              <?php $ran1a = $ranObj->readR($a, $b, $user); while ($row3 = $ran1a->fetch(PDO::FETCH_ASSOC)): ?>
                <td>
                  <?php
                    echo $nor = number_format($row3['alt_an'], 4, '.', ',');
                    /*
          ...
          <tr>
            ...
            <?php $subcri2b = $subcriObj->readAllSubCri($user); while ($row2 = $subcri2b->fetch(PDO::FETCH_ASSOC)): ?>
              ...
              <?php $ran1b = $ranObj->readR($a1, $b2, $user); while ($row3 = $ran1b->fetch(PDO::FETCH_ASSOC)): ?>
                <td>
                  <?php
                    $norx = $row3['alt_an'] * $row2['global_priority_an'];
                    ...
                    //$ranObj->nn4 = $norx;
                    //$ranObj->normalisasi2();
                    $ranObj->insert3($norx, $a1, $b2, $user);
                  ?>
                </td>
              <?php endwhile; ?>
            <?php endwhile; ?>
            <td style="background-color:#E0FFFF">
              <?php
              $stmthasil = $ranObj->readHasil4($a1, $user);
...
<table width="100%" class="table table-striped table-bordered">
          ...
          <tbody>
          <?php $rank = 1; $alt1c = $altObj->readByRank3($user); while ($row = $alt1c->fetch(PDO::FETCH_ASSOC)): ?>
                <tr>
              <!--<td><?=$row["nik"]?></td>-->
              <td><?=$row["alt_name"]?></td>
              <td><?=number_format($row["an_result"], 4, '.', ',')?></td>
