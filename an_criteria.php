...
<div class="content">
  ...
  <div class="form-style-9">
    ...
    <p style="margin-bottom:10px;">
      <strong style="font-size:18pt;"><span class="fa fa-bomb"></span> Pairwise Comparison Result for Criteria (Additive Normalization Method)</strong>
    </p>
<table width="100%" class="table table-striped table-bordered">
        ...
        <tr class="info">
          ...
          <?php $stmt5 = $bobotObj->readAll2($user); while ($row = $stmt5->fetch(PDO::FETCH_ASSOC)): ?>
            <th style="background-color: #FFFFE0">
              <?php
                $bobotObj->readVectorSum($row['fld_criteria_id'], $user);
                echo number_format($bobotObj->nak, 4, '.', ',');
                $bobotObj->insert6($bobotObj->nak, $row['fld_criteria_id'], $user);
              ?>
            </th>
          <?php endwhile; ?>
        </tr>
        ...
        <?php $bobots2x = $bobotObj->readAll2($user); while ($baris = $bobots2x->fetch(PDO::FETCH_ASSOC)): ?>
          <tr>
            ...
            <?php $stmt4x = $bobotObj->readAll2($user); while ($kolom = $stmt4x->fetch(PDO::FETCH_ASSOC)): ?>
              ...
              <?php
                if ($baris['fld_criteria_id'] == $kolom['fld_criteria_id']) {
                  $c = 1- (1/$kolom['criteria_an_total']);
                  $bobotObj->insert7($c, $baris['fld_criteria_id'], $kolom['fld_criteria_id'], $user);
                  echo number_format($c, 4, '.', ',');
                } else {
                  $bobotObj->readAll1($baris['fld_criteria_id'], $kolom['fld_criteria_id'], $user);
                  $c = 1-($bobotObj->kp/$kolom['criteria_an_total']);
                  $bobotObj->insert7($c, $baris['fld_criteria_id'], $kolom['fld_criteria_id'], $user);
                  echo number_format($c, 4, '.', ',');
                }
                ?>
      ...
      <tfoot>
        <tr class="info">
          ...
          <?php $stmt5 = $bobotObj->readAll2($user); while ($row = $stmt5->fetch(PDO::FETCH_ASSOC)): ?>
            <th>
              <?php
                $bobotObj->readSum4($row['fld_criteria_id'], $user);
                echo number_format($bobotObj->hak, 4, '.', ',');
                $bobotObj->insert9($bobotObj->hak, $row['fld_criteria_id'], $user);
              ?>
            </th>
          <?php endwhile; ?>
        </tr>
          ...
          <tr>
            ...
            <?php $stmt4x = $bobotObj->readAll2($user); while ($kolom = $stmt4x->fetch(PDO::FETCH_ASSOC)): ?>
              ...
              <?php
                  ...
                  $bobotObj->readAll4($baris['fld_criteria_id'], $kolom['fld_criteria_id'], $user);
                  $c = ($bobotObj->kp/$kolom['criteria_an_total_2']);
                  $bobotObj->insert8($c, $baris['fld_criteria_id'], $kolom['fld_criteria_id'], $user);
                  echo number_format($c, 4, '.', ',');
            ...
            <th class="info">
              <?php
              $bobotObj->readSum5($baris['fld_criteria_id'], $user);
              $j = $bobotObj->hak;
            ...
            <th class="success">
              <?php
              $bobotObj->readAvg3($baris['fld_criteria_id'], $user);
              $b = $bobotObj->hak;
              $bobotObj->insert10($b, $baris['fld_criteria_id'], $user);
    ...
    <table width="100%" class="table table-striped table-bordered">
      ...
      <tbody>
        <?php $total=0; $bobots1z = $bobotObj->readAll2($user); while ($row1 = $bobots1z->fetch(PDO::FETCH_ASSOC)): ?>
          <tr>
            ...
            $bobotObj->readAvg3($row1['fld_criteria_id'], $user);
              $b = $bobotObj->hak;
              //$bobotObj->insert4($b, $baris['fld_criteria_id']);
              echo number_format($b, 4, '.', ',');
            ?>
          </th>
      ...
      <tfoot>
        <tr>
          ...
          <th><?php $ptot = $total; echo number_format($ptot, 4, '.', ','); ?></th>
        </tr>
      </tfoot>
    </table>
    ...
    <a href="an_sub1.php" class="button">Next</a>
