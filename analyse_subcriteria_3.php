...
include_once('includes/subbobot3.inc.php');
include_once('includes/criteria.inc.php');
//include_once('process.php');

$subbobotObj = new Subbobot3($db);
$criteriaObj = new Criteria($db);
$count = $subbobotObj->countAll();

if(isset($_POST['submit'])){
  $subcriteriaObj = new Subcriteria3($db);
  ...
  while ($row = $subcriterias->fetch(PDO::FETCH_ASSOC)) {
    ...
    $subcriteriaObj->insert($user, $row['fld_subcriteria_id'], $row['fld_subcriteria_name'], 'C3');
    while ($roww = $subcriteriass->fetch(PDO::FETCH_ASSOC)) {
      $pcs = explode("F", $roww['fld_subcriteria_id']);
...
if (isset($_POST['delete'])) {
  ...
  header("location: evaluate_subcriteria_3.php");
}
?>
...
<div class="content">
  ...
  <div class="form-style-9">
     ...
     <ol class="breadcrumb" style="background-color: #F0FFFF;">
      ...
      <li><a href="evaluate_subcriteria.php">Evaluate Sub-Criteria (Universal Design)</a></li>
      <li><a href="analyse_subcriteria.php">Sub-Criteria (Universal Design) Evaluation Table</a></li>
      <li><a href="evaluate_subcriteria_2.php">Evaluate Sub-Criteria (Innovation)</a></li>
      <li><a href="analyse_subcriteria_2.php">Sub-Criteria (Innovation) Evaluation Table</a></li>
      <li><a href="evaluate_subcriteria_3.php">Evaluate Sub-Criteria (Intelligence)</a></li>
      <li class="active">Sub-Criteria (Intelligence) Evaluation Table</li>
    </ol>

   
    <p style="margin-bottom:10px;">
      <strong style="font-size:18pt;"><span class="fa fa-bomb"></span> Pairwise Comparison Result for Sub-Criteria - Intelligence</strong>
    </p>
            ...
            <?php $subbobots3 = $subbobotObj->readAll2($user); while ($kolom = $subbobots3->fetch(PDO::FETCH_ASSOC)): ?>
              <td>
                ...
                if ($baris['fld_subcriteria_id'] == $kolom['fld_subcriteria_id']) {
                  ...
                  $sql = "SELECT * FROM tbl_eduapp_analyse_subcriteria_3 WHERE user_id=$user";
        ...
        <?php $subbobots2x = $subbobotObj->readAll2($user); while ($baris = $subbobots2x->fetch(PDO::FETCH_ASSOC)): ?>
          <tr>
            ...
            <th style="background-color:#FFD700">
              ...
              $conn = new mysqli('lrgs.ftsm.ukm.my', 'a176496', 'bigwhiterabbit', 'a176496');
                  $sql = "SELECT criteria_em FROM tbl_eduapp_criteria_data WHERE fld_criteria_id='C3' AND user_id=$user";
    ...
    <table width="100%" class="table table-striped table-bordered">
      <thead>
        <tr>
          <th>Sub-Criteria</th>
          <?php $subbobots1y = $subbobotObj->readAll2($user); while ($row = $subbobots1y->fetch(PDO::FETCH_ASSOC)): ?>
            <th><?=$row['fld_subcriteria_name'] ?></th>
          <?php endwhile; ?>
          <th class="info">Total</th>
        </tr>
      ...
      <tbody>
        <?php $sumRow = []; $subbobots2y = $subbobotObj->readAll2($user); while ($baris = $subbobots2y->fetch(PDO::FETCH_ASSOC)): ?>
          <tr>
            <th class="active"><?=$baris['fld_subcriteria_name'] ?></th>
            <?php $jumlah = 0; $subbobots3y = $subbobotObj->readAll2($user); while ($kolom = $subbobots3y->fetch(PDO::FETCH_ASSOC)): ?>
              <td>
              <?php
                if ($baris['fld_subcriteria_id'] == $kolom['fld_subcriteria_id']) {
      ...
      <tbody>
        <?php $total=0; $subbobots1z = $subbobotObj->readAll2($user); while ($row1 = $subbobots1z->fetch(PDO::FETCH_ASSOC)): ?>
          <tr>
            ...
            <?php $jumlah = $sumRow[$row1["fld_subcriteria_id"]] + $row1["sub_global_priority"]; ?>
    ...
    <!--<a href="evaluate_subcriteria_3.php" class="button">Next</a>-->
</div>
