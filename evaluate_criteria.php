...
<div class="content">
  ...
  <div class="form-style-9">
     ...
     <ol class="breadcrumb" style="background-color: #F0FFFF;">
      ...
      <li><a href="criteria_data.php">Criteria Data</a></li>
      <li class="active">Evaluate Criteria</li>
    ...
    <p style="margin-bottom:10px;">
      <strong style="font-size:18pt;"><span class="fa fa-bomb"></span> Evaluate Criteria</strong>
    </p>
    <div class="panel panel-default">
      <div class="panel-body">
        <form method="post" action="analyse_criteria.php">
          <div class="row">
          ...
          <?php $no=1; foreach ($r as $k => $v): ?>
            <?php for ($i=1; $i<=$v; $i++): ?>
              <?php $rows = $criteriaObj->readSatu($k, $user); while ($row = $rows->fetch(PDO::FETCH_ASSOC)): ?>
                <div class="row">
                  <div class="col-xs-12 col-md-3">
                    <div class="form-group">
                      <?php $rows = $criteriaObj->readSatu($k, $user); while($row = $rows->fetch(PDO::FETCH_ASSOC)): ?>
                        <input type="text" class="form-control" value="<?=$row['fld_criteria_name'] ?>" readonly />
                        <input type="hidden" name="<?=$k?><?=$no?>" value="<?=$row['fld_criteria_id']?>" />
                      <?php endwhile; ?>
                    </div>
                  </div>
                  <div class="col-xs-12 col-md-6">
                  ...
                  <div class="col-xs-12 col-md-3">
                    <div class="form-group">
                      ...
                      <?php $rows = $criteriaObj->readSatu($nid, $user); while($row = $rows->fetch(PDO::FETCH_ASSOC)): ?>
                        <input type="text" class="form-control" value="<?=$row['fld_criteria_name']?>" readonly />
                        <input type="hidden" name="<?=$nid?><?=$no?>" value="<?=$row['fld_criteria_id']?>" />
                      <?php endwhile; ?>
                    </div>
                  </div>
                </div>
